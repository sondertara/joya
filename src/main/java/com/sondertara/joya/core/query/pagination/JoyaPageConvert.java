...
public class JoyaPageConvert {
   ...
   * @return native sql
   */
  public static NativeSqlQuery buildNativeQuery(
      PageQueryParam queryParam, UnaryOperator<JoinCriterion> joinPart) {
   ...
   */
  public static NativeSqlQuery buildNativeQuery(
      PageQueryParam queryParam, Class<?>... targetClass) {

    return buildNativeQuery(queryParam, null, targetClass);
  }

  @SuppressWarnings("unchecked")
  private static NativeSqlQuery buildNativeQuery(
      PageQueryParam queryParam, UnaryOperator<JoinCriterion> joinFunc, Class<?>... targetClass) {
    List<SearchParam> searchParams = queryParam.getParams();
    UnaryOperator<WhereCriterion> func =
        w -> {
    ...
    for (OrderParam param : list) {
      String orderBy =
          StringFormatter.format("{} {}", param.getFieldName(), param.getOrderType().toString());
