...
public class SqlUtils {
  /** 左边字段提取 */
  public static final Pattern LEFT_COLUMN_FORMAT =
      Pattern.compile(
          "^[t|T][0-9]+(.)[A-Za-z0-9]+[\\s]*[=]?|[\\s][t|T][0-9]+(.)[A-Za-z0-9]+[\\s]*[=]?");
  /** 带表别名的列 */
  public static final Pattern COLUMN_WITH_TABLE_ALIAS =
      Pattern.compile("(?![\"'])[t|T][0-9]+(.)[A-Za-z0-9]+[\\s]*(?![\"'])");
  ...
  public static String buildCountSql(Object strSql) {
    ...
    if (strSql != null) {
      ...
      if (containsDistinctKeywords(sql)) {
        ...
        // 查询字段
        String queryField =
            sql.substring(findStrPosition(sql, "distinct") + 8, findStrPosition(sql, "from"))
                .trim();
  ...
  public static boolean containsDistinctKeywords(String sql) {
    ...
    Pattern pattern =
        Pattern.compile("\\s*" + buildRegexStr("select") + "\\s*" + buildRegexStr("distinct"));
    Matcher matcher = pattern.matcher(sql);
   ...
   * format column name
   *
   * <p>t0.userName ->t0.user_name t0.userId AA user -> t0.user_id AS user
   *
